...
function environment(options) {
  ...
  var deps = {fs: fs, cp: cp, now: function () { return clock; },
    ...
    patch: {TARGET: patch.TARGET, REVISION: 1, ORIGINAL_SHA256: 'original',
...
module.exports = function () {
  ...
  var enabled = e.manager.setEnabled(true);
  ...
  assert.strictEqual(e.commands.filter(function (c) { return c[0] === '/usr/bin/luna-send'; }).length, 1);
  ...
  assert.strictEqual(JSON.parse(e.files[guard.CONFIG].value).enabled, true, 'Monitor suspension preserves opt-in');
  ...
  ['nonroot', 'unsupported', 'foreign', 'spawnFailure'].forEach(function (flag) {
    ...
    assert.throws(function () { rejected.manager.setEnabled(true); }, undefined, flag);
  ...
  [guard.ROOT, guard.OVERLAY, guard.LEASE, guard.STATE, guard.CONFIG].forEach(function (p) {
    ...
    assert.throws(function () { unsafe.manager.setEnabled(true); }, /Unsafe/);
  ...
  assert.throws(function () { hardLink.manager.setEnabled(true); }, /Unsafe/);

  var expiry = environment(); expiry.manager.setEnabled(true);
  ...
  var old = environment(); old.manager.setEnabled(true); old.manager.watchdog('old-generation');
  old.intervals[old.intervals.length - 1].callback(); assert(old.mounted(), 'Stale watchdog cannot unmount a newer generation');
  var uninstall = environment(); uninstall.manager.setEnabled(true);
  ...
  var cannotSave = environment(); cannotSave.manager.setEnabled(true); cannotSave.options.saveFailure = true;
